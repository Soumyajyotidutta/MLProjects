...
ggplot(data=data.frame(Iter=seq(1,i,1), ELBO=elbos[1:i]), aes(x=Iter, y=ELBO)) + geom_line(color="#245FDF")
...
ggplot(df, aes(x=x, color = mu, fill = mu)) + 
         ...
         geom_vline(data = data.frame(x=mk), aes(xintercept=x, color = as.factor(c(2,4,1,3,5))), linetype="dashed", size=1)
